  ...
  <div class="shopOrder">
    ...
    <Modal
      ...
      <Form ref="form" :label-width="100" :model="formData" :rules="formDataRules">
        ...
        <FormItem prop="logisticsFirmObj" label="物流公司">
          <Select v-model="formData.logisticsFirmObj" filterable style="width:200px" placeholder="请输入或选择物流公司">
              <Option v-for="(item, index) in logisticsFirmList" :value="JSON.stringify(item)" :key="index">{{item.name}}</Option>
          </Select>
        </FormItem>
        <FormItem label="重量">
...
import { ApiGetOrderList, ApiExport, API_GETCREATEDISTRIBUTION, ApiGetAlicloudType, ApiUpdateDistributions  } from '@/api'
export default{
  components: {
    Order
  },
  ...
  methods: {
    // 提交配送信息
    submit () {
      let logisticsFirmObj
      console.log(this.formData)
      this.formData['logisticsFirm'] = JSON.parse(this.formData.logisticsFirmObj).name
      this.formData['type'] = JSON.parse(this.formData.logisticsFirmObj).type
      this.formData.weight -=0
      logisticsFirmObj = JSON.stringify(this.formData)
      logisticsFirmObj = JSON.parse(logisticsFirmObj)
      delete logisticsFirmObj.logisticsFirmObj
      const para = {
    ...
    async getAlicloudType () {
      ...
      let alicloudTypeList = []
      for (let i in this.logisticsFirmList) {
        alicloudTypeList.push({
          type: i,
          name: this.logisticsFirmList[i]
        })
      }
      this.logisticsFirmList = alicloudTypeList
    }
  },
  watch: {
    page () {
