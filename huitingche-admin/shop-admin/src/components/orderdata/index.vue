<template>
  <div class="order list">
    <div class="thead">
      <!-- <span v-if="item.key === 'allElection'" v-for="(item, index) in columns" :key="index">1</span> -->
      <span v-for="(item, index) in columns" :key="index" :style="{ width: `${!index ? item.width + 50 : item.width}px` || 'auto' }">
         <Checkbox v-if="item.key === 'all'"  
    ...
    <div v-else v-for="(item, index) in data" :key="index">
      ...
      <div class="info" v-for="(itemO, indexO) in item.orderInfoList" :key="indexO">
        <div class="imgBox">
          <img :src="itemO.goods ? utils.handleImgPrefix(itemO.goods.imgURL.split(',')[0]) : ''" alt="">
        </div>
        <div
          ...
          <Button class="blue" v-if="itemO.logistics !== '无'" type="text" @click="logistics(itemO)">查看物流</Button>
          <Button class="blue" v-if="!distribution" type="text" @click="addLogistics(item.serialnumber)">{{item.state === "已发货" ? '修改' : '填写' }}配送信息</Button>
          <!-- <Button type="text">删除订单</Button> -->
        </div>
        <div
          v-else-if="itemD.title === '全选'"
          :style="{width: itemD.width + 'px' || 'auto'}">
        </div>
        <div
          v-else-if="itemD.title === '商品名称'" 
          :style="{width: (itemD.width - 95) + 'px' || 'auto'}">
    ...
    <Modal
      ...
      <div>
        ...
        <div class="detailInfo">
          <div>订单号：{{orderDetail.serialnumber}}</div>
            <p>购买商品：
          ...
          <div>买家姓名： {{orderDetail.receiptor}}</div>
          <div>买家联系电话： {{orderDetail.phone}}</div>
          ...
          <div>积分抵扣金额: ￥{{orderDetail.integral}}</div>
          <div>实际支付金额: ￥{{(priceSum[detailsIndex] - orderDetail.integral).toFixed(2)}}</div>
        </div>
      </div>
    </Modal>
    <Modal
...
export default {
  ...
  methods: {
    ...
    chooseAll () {
      ...
      // 反向取，因为执行顺序的问题，当点击的时候该判断条件还未做更改，所以取没执行前该值的相反数作为判断条件，即当点击chckebox但该值为ture时表明当前操作是想将他取消。。。
      if (this.checkListall) {
        for (let i in this.checkList) {
    ...
    toDistribution(item) {
      this.$Modal.confirm({
        ...
        onOk: async () => {
          ...
          const data = {
            enterprice: this.$cookies.get("userInfo").id,
            ids: item.id
          };
          ...
          if (res.errorCode > 0) {
            ...
            this.$Modal.confirm({
              content: "该订单已加入配送单，是否去已发货订单填写配送信息？",
              onOk: () => {
